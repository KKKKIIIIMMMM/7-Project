...
class view: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate {
    ...
    override func viewDidLoad() { // 함수의 이름은 뷰디드로드이고 파라미터의 반환타입은 없습니다.
        ...
        cell.separatorColor = UIColor.gray // 셀 사이의 구분선 색상을 설정합니다.
        ...
        cell.register(CustomTableViewCell.self, forCellReuseIdentifier: "CustomTableViewCell") // 테이블 뷰에서 사용할 셀의 클래스 또는 Nib 파일을 등록합니다.
    ...
    // 09.26 오전 숙제
    // 1. 만약에 issort가 true인 경우 false로 변경해주세요.
    // 2. issort가 false일때는 true로 변경해주세요.
    // 3. 그 다음에 reloadData 함수를 호출해주세요.
    
    // unc reloadData() { // 함수의 이름은 reloadData의 memorist 값 cdm의 loadMemo를 불러옵니다.
//    memorist = cdm.loadMemo()
//    if issort == true{
//        memorist.sort { a, b in
//            b.time < a.time
...
//        let memo = memoArray[indexPath.row]
        cell.delegate = self
//        if let savedColor = memo.textColor {
//            cell.textLabel?.textColor = savedColor
//        }
        // 테이블뷰의 셀 제목은 현재 모른다. 제목 셀행의 선택이 가능하다.
//        / 09.26 오후 숙제
//        1.
//        - memorist[indexPath.row]의 color가 "#000000" 와 같다면 cell의 title의 textColor에 .gray를 할당해주세요.
//
//        - 만약 "#000000"과 같지 않다면 cell의 title의 textColor에 UIColor.hexStringToUIColor(hex: memorist[indexPath.row].color) 를 할당해주세요.
//
//        2. 위에 isFiltering이 true일 때도 작업해주세요.
        if isFiltering {
...
//            filteredItems = memorist.filter { $0.lowercased().contains(searchText.lowercased()) }
            ...
            for memo in memorist  {
                // lowercased()는 언어 설정에 맞게 현지화된 방식으로 소문자 변환을 지원합니다.
                // contains(_:)는 단순히 해당 요소가 포함되어 있는지 확인하는 데 사용됩니다.
                // memo의 memorist를 반복하고 검색어의 소문자 변환을 지원 하고 검색된 값을 보냅니다.
    ...
    // IndexPath테이블 뷰가 쿼리하는 섹션과 행을 지정하는 데 사용됩니다. 그것은 두 가지 주요 속성을 가지고 있다
    // row 는 테이블 데이터에서 특정 항목의 인덱스 또는 위치를 나타냅니다.
    // didSelectRowAt는 iOS의UITableView에서 사용되는 위임 메서드로, 사용자가 테이블 뷰의 셀을 선택했을 때 호출됩니다. 이 메서드는 셀을 선택한 후 발생할 동작을 처리하는 데 사용되며, 주로 화면 전환, 데이터 업데이트, 상세 정보 표시 등을 구현할 때 활용됩니다.
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        // deselectRow는 iOS의UITableView에서 선택된 셀의 선택 상태를 해제하는 메서드입니다.
...
//        viewcontroller.entity = memorist[selectedRow] // memorist 배열의 selectedRow번째의 값을 viewController의 entity 에 할당한다.
        // present는뷰 컨트롤러를 화면에 표시하기 위해 사용하는 메서드입니다.
...
// 만약에 엔티티의 체크가 같다면 스트링체크와 같다면 체크메모를 스트링값을 입력한다. 만약에 아니면 지금과 같이 체크메모함수를 사용해서 동일하게 사용한다.
